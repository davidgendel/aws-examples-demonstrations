...
class DeploymentValidator:
    ...
    def check_system_requirements(self) -> bool:
        ...
        try:
            result = subprocess.run(['python3', '-m', 'venv', '--help'], capture_output=True, text=True)
            if result.returncode == 0:
                self.print_success("Virtual environment support available")
                checks_passed += 1
            else:
                self.print_error("Virtual environment not supported")
        except:
            self.print_error("Cannot create virtual environments")
        
        success_rate = (checks_passed / total_checks) * 100
        print(f"\n{Colors.CYAN}System Requirements: {checks_passed}/{total_checks} checks passed ({success_rate:.0f}%){Colors.NC}")
        
        return checks_passed >= 5  # Allow 1 failure
