    ...
    if python_version:
        ...
        if version_match:
            ...
            if major < 3 or (major == 3 and minor < 13):
                warning(f"Python version {major}.{minor}.{patch} detected. This project recommends Python 3.13 or higher.")
            else:
...
def build_project():
    ...
    if Path("setup.py").exists():
        ...
        warning("No setup.py found, skipping build step")
...
def deploy_infrastructure(region: str):
    ...
    run_command(f"python -m src.infrastructure.app")
...
def process_knowledge_base(region: str):
    ...
    if docs_folder.exists() and any(docs_folder.iterdir()):
        ...
        try:
            run_command(f"python -m scripts.upload_documents --folder {DOCS_FOLDER}")
...
def display_summary(outputs: Dict[str, str]):
    ...
    section("Next Steps")
    print(f"1. Add more documents to the '{DOCS_FOLDER}' folder and run 'python -m scripts.upload_documents --folder ./{DOCS_FOLDER}'")
